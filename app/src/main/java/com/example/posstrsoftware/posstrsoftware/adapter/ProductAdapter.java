        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            if (constraint != null && constraint.length() > 0) {
                ...
                for (int i = 0; i < filterList.size(); i++) {
                    if (filterList.get(i).getProductText().toUpperCase().contains(constraint)||filterList.get(i).getBarcode().toUpperCase().contains(constraint)) {
                        ProductList u = new ProductList( filterList.get(i).getProductText(),
                                filterList.get(i).getProductprice(), filterList.get(i).getGroupList(),
                                filterList.get(i).getUnitList(),filterList.get(i).getCost(),
                                filterList.get(i).getBarcode(), filterList.get(i).getSymbolVat(),
                                filterList.get(i).getCheckvat(),filterList.get(i).getId());
                        filters.add(u);
                    }
                }
