...
public class DbHelper extends SQLiteOpenHelper {
    ...
    private static final String viewProductList = "CREATE VIEW vproduct_list as select *  ,CASE WHEN vat_flag = 'Y' THEN   " +
            " round(((cast(price_text as decimal)/(select (vatrate +100.0)/100 from company_list limit 1))),2) " +
            " ELSE cast(price_text as decimal) END AS PriceNoVAT ,  " +
            " CASE WHEN vat_flag = 'Y' THEN round(((cast(price_text as decimal)   " +
            " *(select vatrate /100.0 from company_list limit 1))/(select (vatrate +100.0)/100 from company_list limit 1)),2)ELSE " +
            " cast(0.00 as decimal) END AS   ValueVat  from product_list order by 1";


    private static final String viewmasterList = "CREATE VIEW viewmaster_list as select sale_master_id,doc_date,sum(cast(productprice_text as decimal)) as PriceNoVat,cast(discount as decimal)\n" +
            "    as bill_discount,count(sale_master_id) as count_amount, sum(cast(product_price as decimal)) as PriceSumVat,\n" +
            "    sum(cast(product_cost as decimal)) as sum_product_cost , \n" +
            "    sum(Value_Vat) as sum_vat,\n" +
    ...
    private static final String viewdetailList = "CREATE VIEW viewdetail_list as  select productsale_text" +
            ",sum(cast(product_price as decimal)) as sum_product_price" +
            ",count(productsale_text) as product_amount from transectionBill" +
            " where sale_master_id = (select sale_master_id from viewmaster_list  " +
